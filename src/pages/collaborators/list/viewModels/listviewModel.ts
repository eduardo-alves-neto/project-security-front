...
import type { ListModelType } from "../types"
import type { ICollaborator } from "../../services/types"
import { enqueueSnackbar } from "notistack"
import { columns } from "../constants/columns"

export const useListViewModel = ({ listDataQuery, mutationDelete }: ListModelType) => {
  ...
  const fetch = useCallback(() => {
    mutateAsync()
      ...
      .catch((error) => {
        enqueueSnackbar(error.response?.data?.message || "Erro ao carregar colaboradores", { variant: "error" })
      })
  }, [])
  ...
  const handlerDelete = useCallback(
    async (id: string) => {
      try {
        ...
        enqueueSnackbar(error.response?.data?.message || "Erro ao deletar colaborador", { variant: "error" })
      }
    },
    [fetch],
  )
